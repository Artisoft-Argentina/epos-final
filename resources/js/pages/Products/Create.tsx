...
import { ChevronRight, Save, Package, DollarSign, QrCode, ImagePlus, X, Warehouse } from 'lucide-react';
import { useState } from 'react';

interface Category { id: number; name: string; }
interface Brand { id: number; name: string; }
interface Supplier { id: number; business_name: string; }
interface Props { categories: Category[]; brands: Brand[]; suppliers: Supplier[]; }

const UNITS = ['Unidad', 'Kg', 'Litro', 'Metro', 'Caja', 'Pack', 'Par', 'Rollo'];

export default function Create({ categories, brands, suppliers }: Props) {
                ...
                <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
                    <div>
                        <div className="flex items-center gap-1.5 text-sm text-muted-foreground mb-1">
                            <Link href={route('products.index')} className="hover:text-primary transition-colors">
                                Productos
                            </Link>
                            ...
                            <Card className="gap-0 py-0">
                                ...
                                <CardContent className="px-6 py-5 grid grid-cols-1 md:grid-cols-2 gap-5">
                                    ...
                                    <FormField
                                        ...
                                        htmlFor="sku"
                                        error={errors.sku}
                                        hint={autoSku ? 'Se generará automáticamente al guardar.' : 'Código interno único del producto.'}
                                    >
                                        <div className="flex items-center gap-3">
                                            <div className="flex items-center gap-2 shrink-0">
                                                <Switch
                                                    id="auto-sku"
                                                    checked={autoSku}
                                                    onCheckedChange={handleAutoSku}
                                                />
                                                <label htmlFor="auto-sku" className="text-sm text-muted-foreground cursor-pointer select-none">
                                                    Automático
                                                </label>
                                            </div>
                                            <Input
                            ...
                            <Card className="gap-0 py-0">
                                ...
                                <CardContent className="px-6 py-5 flex flex-col gap-5">
                                    <div className="flex items-center justify-between p-4 rounded-lg border border-border bg-muted/30">
                                        ...
                                        <Switch
                                            checked={trackStock}
                                            onCheckedChange={handleTrackStock}
                                        />
                                    </div>
                        ...
                        <div className="flex flex-col gap-6">
                            ...
                            <Card className="gap-0 py-0">
                                ...
                                <CardContent className="px-6 py-5 flex flex-col gap-4">
                                    <label
                                        htmlFor="images"
                                        className="flex flex-col items-center justify-center gap-2 rounded-lg border-2 border-dashed border-border bg-muted/30 p-6 text-center cursor-pointer hover:bg-muted/50 transition-colors"
                                    >
