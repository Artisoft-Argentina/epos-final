...
export default function Create({ warehouses }: Props) {
    const { data, setData, post, processing, errors } = useForm({
        ...
        next_invoice_number_a: '1',
        next_invoice_number_b: '1',
        next_invoice_number_c: '1',
        is_default: false,
        active: true,
    });
    ...
    const submit = (e: React.FormEvent) => {
        e.preventDefault();
        post(route('puntos-venta.store'));
    };

    return (
        <AppLayout>
            <Head title="Crear Punto de Venta" />
            <div className="p-6">
                <Card className="max-w-2xl">
                    <CardHeader><CardTitle>Crear Nuevo Punto de Venta</CardTitle></CardHeader>
                    <CardContent>
                        <form onSubmit={submit} className="space-y-4">
                            <FormField label="Nombre" htmlFor="name" error={errors.name} required>
                                <Input id="name" value={data.name} onChange={(e) => setData('name', e.target.value)} placeholder="Ej: Caja 1 - Centro" error={errors.name} />
                            </FormField>
                            <FormField label="Número de Punto de Venta (ARCA)" htmlFor="pos_number" error={errors.pos_number} required>
                                <Input id="pos_number" type="number" min="1" max="99999" value={data.pos_number} onChange={(e) => setData('pos_number', e.target.value)} placeholder="Ej: 2" error={errors.pos_number} />
                            </FormField>
