...
export default function Create() {
    ...
    const submit = (e: React.FormEvent) => { e.preventDefault(); post(route('marcas.store')); };
                    ...
                    <CardContent>
                        <form onSubmit={submit} className="space-y-4">
                            <FormField label="Nombre" htmlFor="name" error={errors.name} required>
                                <Input id="name" value={data.name} onChange={(e) => setData('name', e.target.value)} placeholder="Ingrese el nombre de la marca" error={errors.name} />
                            </FormField>
